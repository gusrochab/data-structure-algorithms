...
def create_person_objects(name, age, month):
    data = zip(name, age, month)
    person_list = []
    for item in data:
        person_list.append(Person(*item))
    return person_list

def get_april_birthdays(person_list):
    birthday_list = []
    for person in person_list:
        person.birthday()
        if person.month == 'April':
            birthday_list.append([person.name, person.age])
    return dict(birthday_list)

def get_most_common_month(person_list):
    birthday_months = []
    for person in person_list:
        birthday_months.append(person.month)
    months = list(set(birthday_months))
    most_common = months[0]
    for month in months:
        if birthday_months.count(month) > birthday_months.count(most_common):
            most_common = month
    return most_common
